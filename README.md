...
- [x] comparison execution time for different runs (isolation tree, kmeans tree, isolation ensemble, kmeans ensemble)
- [x] parallelize the ensembles (both isolation and kmeans)

- [x] compare times execution with already existing implementations (only isolation tree (can use forest with t=1) and isolation ensemble) (these two are from sklearn)
...
- [x] comparison with original authors (if they have any implementations) (both papers don't seem to have)

- [x] sensibility to parameters (higher contaminations for real datasets, ensemble size, subsampling size?)
